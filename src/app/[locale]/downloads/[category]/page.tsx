...
import { downloadSectionTitle, findDownloadSectionBySlug } from "@/lib/download-sections";
...
export async function generateMetadata({
  ...
  if (config) {
    return {
      title: downloadCategoryLabel(category, appLocale),
      alternates: buildAlternates(`/downloads/${category}`),
    };
  }

  const section = await findDownloadSectionBySlug(category);
  if (section?.sectionType === "CUSTOM") {
    return {
      title: downloadSectionTitle(section, appLocale),
      alternates: buildAlternates(`/downloads/${category}`),
    };
  }
...
export default async function DownloadCategoryPage({
  ...
  if (config) {
    ...
    return (
      <>
        ...
        <ResourceList resources={resources} appLocale={appLocale} isFa={isFa} />
      </>
    );
  }
  ...
  return (
    <>
      <PageHeader title={downloadSectionTitle(section, appLocale)} />
      <ResourceList resources={resources} appLocale={appLocale} isFa={isFa} />
    </>
  );
}
...
function ResourceList({
  ...
  resources: Awaited<ReturnType<typeof prisma.downloadResource.findMany>>;
  appLocale: AppLocale;
  isFa: boolean;
}) {
  return (
