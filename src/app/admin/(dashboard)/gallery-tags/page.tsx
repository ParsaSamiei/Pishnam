...
import { deleteGalleryTag } from "./actions";
...
export default async function AdminGalleryTagsPage() {
  ...
  return (
    <div>
      <AdminListHeader
        title="برچسب‌های گالری"
        newHref="/admin/gallery-tags/new"
        newLabel="افزودن برچسب"
      />
      <p className="text-text-secondary mt-2 text-sm">
        برچسب‌ها به‌صورت فیلتر روی صفحه عمومی{" "}
        <span dir="ltr" className="font-mono text-xs">
          /gallery
        </span>{" "}
        نمایش داده می‌شوند. موارد گالری را از بخش{" "}
        <Link href="/admin/gallery" className="text-pishnam-gold-600 underline">
          گالری تصاویر
        </Link>{" "}
        به این برچسب‌ها وصل کنید.
      </p>
            ...
            },
            {
              ...
              cell: (row) => (
                <div className="flex justify-end gap-1">
                  <Link
                    href={`/admin/gallery-tags/${row.id}/edit`}
                  ...
                  <DeleteButton
                    onDelete={deleteGalleryTag.bind(null, row.id)}
