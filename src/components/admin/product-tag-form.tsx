...
interface ProductTagFormProps {
  action: (prevState: ProductTagFormState, formData: FormData) => Promise<ProductTagFormState>;
  defaultValues?: {
    slug: string;
    nameFa: string;
    nameEn: string;
...
const initialState: ProductTagFormState = { status: "idle" };

export function ProductTagForm({ action, defaultValues, submitLabel }: ProductTagFormProps) {
