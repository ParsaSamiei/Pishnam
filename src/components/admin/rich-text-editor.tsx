 ...
 * Pishnam's stack"). The live HTML is held in React state and rendered as the
 * *controlled* value of a hidden input, so it works inside a plain
 * <form action={serverAction}> without any client-side submit wiring -- the
 * server action reads `name` out of FormData exactly like any other field,
 * then it is run through sanitizeRichText() before being rendered (see
 * components/rich-text.tsx) -- this editor's own allowlisted extensions are a
 * UX constraint, not the security boundary.
 *
 * The hidden input must stay controlled (`value`, driven by state) and must
 * never be an uncontrolled `defaultValue` that an effect writes `.value` into.
 * For `type="hidden"` the `value` IDL attribute reflects the *content
 * attribute*, so React reconciling the `defaultValue` prop overwrites whatever
 * an effect assigned -- and Tiptap re-renders on every keystroke to refresh the
 * toolbar's active states. That silently submitted an empty body on create and
 * reverted to the previously saved body on edit, while the author still saw
 * their text in the (non-form-control) contenteditable.
 * See rich-text-editor.test.tsx.
 */
export function RichTextEditor({ name, defaultValue = "", error }: RichTextEditorProps) {
  ...
  const editor = useEditor({
    ...
    // An empty document stringifies to "<p></p>", which would sail past the
    // schema's required check and store a blank body -- submit "" instead so
    // the field is reported as missing.
  ...
  return (
    <div>
      {/* Rendered unconditionally: the editor mounts in an effect
          (immediatelyRender: false), and a missing input would submit the field
          as undefined rather than as an empty string. */}
      <input type="hidden" id={`richtext-${name}`} name={name} value={html} />
      {editor && (
        <>
